...
import { setResumeToolbarKeys } from '@src/store/features/templateSlice';

function Toolbar() {
  const height = document.body.clientHeight;
  const dispatch = useAppDispatch()
  // const { value } = useAppSelector((store) => store.template)
  ...
  const changeResumeToolbarKeys = (moduleKeys: string[]) => {
    if (moduleKeys.length > 0) {
      dispatch(setResumeToolbarKeys({ value: moduleKeys }))
    }
  };
